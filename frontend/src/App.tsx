...
export const App: React.FC = () => {
  ...
  // Detectar pausa prolongada al inicio
  useEffect(() => {
    if (!startedAt) return;
  ...
  const calculateTypingMetrics = (): TypingMetrics => {
    ...
    // Calcular pausas entre campos (convertir a enteros como espera el backend)
    const pauses: number[] = [];
    ...
    for (let i = 0; i < fields.length - 1; i++) {
      ...
      if (currentEnd && nextStart) {
        // Convertir a entero (segundos)
        pauses.push(Math.round((nextStart - currentEnd) / 1000));
      }
    }

    return {
      keystrokes_per_second: Math.round(keystrokesPerSecond * 100) / 100, // Redondear a 2 decimales
      backspaces_count: backspaces,
      pauses_between_fields: pauses.length > 0 ? pauses.map(p => Math.round(p)) : [0], // Convertir a enteros
  ...
  const handleChange = (field: keyof StarState, value: string) => {
    ...
    // Iniciar temporizador de escritura si es la primera vez
    if (!typingStartTime) {
      setTypingStartTime(Date.now());
    }

    // Registrar inicio de campo si es la primera vez que se edita
    if (!fieldStartTimes[field]) {
      setFieldStartTimes((prev) => ({ ...prev, [field]: Date.now() }));
    } else {
      // Actualizar tiempo de fin del campo anterior
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Asegurar que suggestions siempre sea un array
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-neutral-50">
      <main className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-12">
        ...
        <header className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold text-neutral-900">
              T2.1 · Describe un proyecto o experiencia
            </h1>
          </div>
        ...
        <footer className="text-center text-sm text-neutral-500 pt-4 border-t border-neutral-200">
          Este módulo solo entrena la etapa <strong>Experiencia (T2.1)</strong>.
          Usa estas ideas como borrador; la práctica real será en voz alta.
        </footer>
      </main>
    </div>
  );
